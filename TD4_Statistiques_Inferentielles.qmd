...
![Loi Binomiale pour $n=3$](image/arbre_binomial.png){fig-align="center" width="100%"}
...
::: callout-note
## Exercice 2
Estimer l'intervalle de confiance par la méthode de Monte Carlo (on pourra faire le graphique de la distribution des moyennes). 
:::
...
ic<-round(quantile(moy_est,probs=c(alpha/2,1-alpha/2)),2)
ggplot(data.frame(moy_est), aes(x = moy_est)) +
  geom_histogram(aes(y = after_stat(density)),
                 bins = 50,
                 fill = "#3498db", color = "white", alpha = 0.7) +
  labs(
    x        = "Bootstrap non paramétrique",
    y        = "Densité",
    title    = "Distribution de la moyenne empirique par bootstrap non para.",
    subtitle = paste("IC=[",ic[[1]],",",ic[[2]],"]",sep=""))+
...
::: callout-note
## Exercice 3
Estimer l'intervalle de confiance par Bootstrap paramétrique (on fera le graphique de la distribution des moyennes). 
:::
...
ggplot(data.frame(moy_est), aes(x = moy_est)) +
  ...
  labs(
    x        = "Bootstrap non paramétrique",
    y        = "Densité",
    title    = "Distribution de la moyenne empirique par bootstrap  para.",
    subtitle = paste("IC=[",ic[[1]],",",ic[[2]],"]",sep=""))+
  theme_bw(base_size = 12) +
  theme(
    plot.title       = element_text(face = "bold"),
...
On considère un échantillon de valeurs $x_1,...,x_n$ issue d'une variable aléatoire $X$ et on veut estimer la distribution de $\bar X_n$.

Répéter $B$ fois les étapes ci-dessous :
Etape 1 : On tire avec remise dans l'échantillon $x_1,...,x_n$, l'échantillon obtenu est noté $x^*_1,...,x^*_n$.
Etape 2 : On calcule la moyenne $\bar X^*_n$ de l'échantillon obtenu.

Les $B$ moyennes obtenues permettent d'obtenir une approximation de la distribution de $\bar X_n$.
:::
...
```{r echo=F}
ic<-round(quantile(moy_star,probs=c(alpha/2,1-alpha/2)),2)
ggplot(data.frame(moy_star), aes(x = moy_star)) +
  geom_histogram(aes(y = after_stat(density)),
                 bins = 50,
                 fill = "#3498db", color = "white", alpha = 0.7) +
  #stat_function(fun  = dnorm,
